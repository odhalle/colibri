...
exports.create = function (req, res) {
    ...
    extension.save(function (err, data) {
        if (err) {
            console.log(err);
            res.status(500).send({ message: "Some error occurred while creating the extension." })
        } else {
            res.send(data);
        }
    });
...
exports.findAll = function (req, res) {
    ...
    Extension.find(function (err, extensions) {
        if (err) {
            console.log(err);
            res.status(500).send({ message: "Some error occurred while retrieving extensions." });
        } else {
            res.send(extensions);
        }
    });
    ...
    Extension.findById(req.params.extensionId, function (err, extension) {
        if (err) {
            ...
            if (err.kind === 'ObjectId') {
                return res.status(404).send({ message: "Extension not found with id " + req.params.extensionId });
            }
            return res.status(500).send({ message: "Error retrieving extension with id " + req.params.extensionId });
        }
...
exports.update = function (req, res) {
    ...
    Extension.findById(req.params.extensionId, function (err, note) {
        if (err) {
            console.log(err);
            if (err.kind === 'ObjectId') {
                return res.status(404).send({ message: "Extension not found with id " + req.params.extensionId });
            }
            return res.status(500).send({ message: "Error finding extension with id " + req.params.extensionId });
        }
        ...
        extension.save(function (err, data) {
            if (err) {
                res.status(500).send({ message: "Could not update extension with id " + req.params.extensionId });
            } else {
                res.send(data);
            }
        });
...
exports.delete = function (req, res) {
    ...
    Extension.findByIdAndRemove(req.params.extensionId, function (err, extension) {
        if (err) {
            ...
            return res.status(500).send({ message: "Could not delete extension with id " + req.params.extensionId });
        }
